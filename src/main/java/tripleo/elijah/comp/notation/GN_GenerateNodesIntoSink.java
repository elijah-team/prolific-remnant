...
import tripleo.elijah.nextgen.rosetta.*;
...
public class GN_GenerateNodesIntoSink implements GN_Notable, CompilationEnclosure.ModuleListener {
	...
	public void run() {
		...
		env.moduleList().getMods().stream().forEach(mod -> run_one_mod(mod, wm));
	...
	private void run_one_mod(final OS_Module mod, final WorkManager wm) {
		final var lang0 = GN_GenerateNodesIntoSinkEnv.getLang(mod);
		if (lang0.mode() == Mode.FAILURE) {
			throw new ProgramIsWrongIfYouGetHere(); // 11/24 i think...
		}

		final GM_GenerateModuleRequest gmr = new GM_GenerateModuleRequest(this, mod, env);
		Rosetta.create(gmr, wm, env.resultSink1()).apply();
	}
