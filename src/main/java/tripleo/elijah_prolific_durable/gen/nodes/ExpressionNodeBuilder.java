...
public class ExpressionNodeBuilder {
	...
	public static IExpression binex(final TypeRef rt, final VariableReference left, final ExpressionOperators middle, final TmpSSACtxNode right) { // todo wrong again
		// TODO Auto-generated method stub
		final ExpressionKind middle1 = Helpers.ExpressionOperatorToExpressionType(middle);
		return new BasicBinaryExpression(left, middle1, new StringExpression(tripleo.elijah_prolific_durable.util.Helpers.makeToken(right.text()))); // TODO !!!
	}
	...
	public static IExpressionNode fncall(final MethRef aMeth, final List<LocalAgnTmpNode> of) { // TODO no so wrong anymore
		...
		xyz.append(tripleo.elijah_prolific_durable.util.Helpers.string_to_ident(aMeth.getTitle()));
		pce1.identifier(xyz);
		//
		//
...
////					sb.append(',');
//				}
				sb.append(tripleo.elijah_prolific_durable.util.Helpers.String_join(",", Collections2.transform(pce1.getArgs().expressions(), new Function<IExpression, String>() {
					@Nullable
					@Override
...
//		final Token t = tripleo.elijah.util.Helpers.makeToken(string);
		xyz.append(tripleo.elijah_prolific_durable.util.Helpers.string_to_ident(string));
		pce1.identifier(xyz);
		//
		final ExpressionList expl = Helpers.LocalAgnTmpNodeToListVarRef(of);
		pce1.setArgs(expl);
...
//	}
	...
	public static IdentExpression ident(final String string) {
		...
		return new IdentExpression(tripleo.elijah_prolific_durable.util.Helpers.makeToken(string));
