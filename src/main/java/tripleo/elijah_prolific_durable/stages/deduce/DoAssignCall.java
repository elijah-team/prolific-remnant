...
import tripleo.elijah_prolific_durable.stages.deduce.declarations.DeferredMemberFunction;
...
//								vte.setStatus(BaseTableEntry.Status.KNOWN, best);
							...
							@NotNull final VariableTableEntry   vte2    = generatedFunction.getVarTableEntry(DeduceTypes2.to_int(vte2_ia));
							if (p.isResolved())
		...
		try {
			...
			if (best != null) {
				if (best instanceof @NotNull final VariableStatement vs) { // TODO what about alias?
					...
					if (vte_ia != null) {
						@NotNull final VariableTableEntry vte1 = generatedFunction.getVarTableEntry(DeduceTypes2.to_int(vte_ia));
						throw new NotImplementedException();
					} else {
...
//							ty = idte.type.getAttached();
					...
					@Nullable final InstructionArgument vte_ia = generatedFunction.vte_lookup(s);
					if (vte_ia != null) {
						@NotNull final VariableTableEntry vte2 = generatedFunction.getVarTableEntry(DeduceTypes2.to_int(vte_ia));
